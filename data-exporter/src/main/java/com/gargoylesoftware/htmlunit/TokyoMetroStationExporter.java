...
import com.gargoylesoftware.htmlunit.TokyoMetroStation.Station;

public class TokyoMetroStationExporter {
	...
	public static void main(final String[] args)
		...
		//
		final List<Method> methods = toMethods(args, TokyoMetroStationExporter.class.getDeclaredMethods());
		//
		Method m = null;
		//
		for (int i = 0; methods != null && i < methods.size(); i++) {
			...
			if (Arrays.equals(m.getParameterTypes(), new Class[] { File.class, List.class })) {
				//
				if (stations == null) {
					stations = TokyoMetroStation.getStations();
				}
				//
				m.invoke(null,
