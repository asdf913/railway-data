...
import com.gargoylesoftware.htmlunit.HankyuStation.Station;

public class HankyuStationExporter {
	...
	public static void main(final String[] args)
		...
		//
		final List<Method> methods = toMethods(args, HankyuStationExporter.class.getDeclaredMethods());
		//
		Method m = null;
		//
		for (int i = 0; methods != null && i < methods.size(); i++) {
			...
			if (Arrays.equals(m.getParameterTypes(), new Class[] { File.class, List.class })) {
				//
				if (stations == null) {
					stations = HankyuStation.getStations();
				}
				//
				m.invoke(null,
